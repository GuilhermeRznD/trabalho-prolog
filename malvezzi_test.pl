...
%DEFINICAO DE NASCIMENTO
nasceu(denis, janeiro).
nasceu(joao, fevereiro).
nasceu(lenin, maio).
nasceu(otavio, setembro).
nasceu(will, dezembro).

%GOSTO MATERIA
gosta(joao, historia).
gosta(denis, matematica).
gosta(lenin, dama).
gosta(will, logica).
gosta(otavio, geografia).

%GOSTO JOGOS
gosta(joao, '3_ou_mais').
gosta(denis, 'cubo_vermelho').
gosta(lenin, 'jogo_da_forca').
gosta(will, 'prob_de_logica').
gosta(otavio, 'caca_palavras').
...
modelo([
    ...
    % Definindo os domínios para as variáveis
    ...
    % Garantindo que todos os valores sejam diferentes
    ...
    mochila(azul), algum_lugar_a_direita(Mochila1, Mochila2), nasceu(Nome2, maio),
    ...
    jogo(jogo_da_forca), mochila(vermelha), ao_lado(Jogo1, Mochila1),
